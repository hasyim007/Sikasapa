/* =========================================================
   ...
   login.html, admin.html). Sumber data: Supabase (Postgres),
   lewat fungsi RPC (lihat config.js & supabase-schema.sql).
   ========================================================= */
function seedDB(){
  return {
    ekskul: [
...
/* =========================================================
   SIKasapa — DATA LAYER (Supabase)
   Ganti dari localStorage: DB sekarang dimuat & disimpan lewat
   Supabase (lihat config.js untuk SUPABASE_URL/SUPABASE_ANON_KEY).
   ========================================================= */
...
/* Klien Supabase (URL & anon key ada di config.js). Semua akses data
   HANYA lewat fungsi RPC yang didefinisikan di supabase-schema.sql —
   lihat file itu untuk penjelasan kenapa (RLS menutup tabel asli,
   fungsi RPC yang jalan sebagai SECURITY DEFINER yang memvalidasi
   token/password di sisi server, mirip peran Code.gs dulu). */
...
/* saveDB tetap SINKRON dari sisi pemanggil (mengikuti ratusan titik
   ...
   Supabase berjalan di background lewat RPC save_all. */
function saveDB(db){
  try{
...
/* Unggah gambar (logo / bukti pengeluaran) ke Supabase Storage
   (bucket "lampiran"), menggantikan Google Drive. Server tetap
   memvalidasi token & role (harus Bendahara) lewat RPC
   request_upload_ticket sebelum file diterima — lihat
   supabase-schema.sql bagian TIKET UPLOAD. */
async function uploadFileToServer(dataUrl, filename, mimeType){
  const tiket = await rpc('request_upload_ticket', { p_token: getToken()||'' });
  if(!tiket.ok) throw new Error(tiket.error || 'Tidak bisa memulai upload.');
  const base64 = String(dataUrl).split(',')[1] || '';
  if(!base64) throw new Error('File kosong.');
  const bytes = Uint8Array.from(atob(base64), c => c.charCodeAt(0));
  const namaAman = (filename||'file').replace(/[^a-zA-Z0-9._-]/g,'_');
  const path = `${tiket.ticket}/${Date.now()}-${namaAman}`;
  const { error } = await supabaseClient.storage.from('lampiran').upload(path, bytes, { contentType: mimeType || 'image/png', upsert:false });
  if(error) throw new Error(error.message || 'Upload gagal');
  const { data } = supabaseClient.storage.from('lampiran').getPublicUrl(path);
...
function bacaPxBuktiFile(file){
  ...
  reader.onload = async ()=>{
    _pxBuktiData = reader.result; // preview lokal instan
    updatePxBuktiUI();
    try{
      const url = await uploadFileToServer(reader.result, file.name, file.type);
      _pxBuktiData = url; // ganti ke URL Supabase Storage setelah terunggah
    }catch(err){
      console.error(err);
      showToast('Gagal mengunggah bukti ke server, coba lagi.', 'error');
      _pxBuktiData = null;
      updatePxBuktiUI();
    }
  };
  ...
  main.innerHTML = `
    <div class="glass-strong rounded-3xl p-5 mb-5 flex items-center gap-4">
      ...
      <div>
        ...
        <p class="text-xs text-slate-500">Rekap hanya berlaku untuk ekstrakurikuler dengan skema pembayaran bulanan.</p>
      </div>
    </div>
...
function nomorLaporan(ek){
  const now = hariIniDate();
  const kode = (ek.nama||'EK').toUpperCase().replace(/[^A-Z]/g,'').slice(0,4) || 'EK';
  return `001/SIKASAPA-${kode}/${romawiBulan(now.getMonth()+1)}/${now.getFullYear()}`;
}

function cetakLaporan(ekskulId){
  const ek = ekskulById(ekskulId);
  const pg = DB.pengaturan;
  ...
  w.document.write(`
    ...
    <body>
      ...
      <p style="font-size:11px; color:#555;">Nomor Laporan: ${nomorLaporan(ek)} · ${escapeHtml(labelPeriodeLaporan())}</p>
  ...
  `);
  w.document.close();
  catatAktivitas('Cetak Laporan', `${ek.nama} — ${nomorLaporan(ek)} — ${labelPeriodeLaporan()}`);
  saveDB(DB);
...
/* jsPDF butuh gambar dalam bentuk data URL (base64). Logo/bukti kini
   tersimpan sebagai URL publik Supabase Storage, yang mengizinkan
...
async function unduhLaporanPdf(ekskulId){
  ...
  doc.text(`Nomor Laporan: ${nomorLaporan(ek)}  ·  ${labelPeriodeLaporan()}`, 14, y);
  doc.setTextColor(0);
  y += 5;
  doc.setFontSize(9);
  ...
  catatAktivitas('Unduh Laporan PDF', `${ek.nama} — ${nomorLaporan(ek)} — ${labelPeriodeLaporan()}`);
  saveDB(DB);
...
function simpanLogoWebFile(file){
  ...
  reader.onload = async ()=>{
    try{
      ...
      console.error(err);
      showToast('Gagal mengunggah logo web ke server.', 'error');
    }
  };
  reader.readAsDataURL(file);
}
  ...
  main.innerHTML = `
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
      ...
      <div class="glass-strong rounded-3xl p-6">
        ...
        <p class="text-xs text-slate-500 mb-4">Data utama sudah tersimpan di database Supabase. Unduh backup .json sesekali sebagai cadangan tambahan, terutama sebelum melakukan perubahan besar.</p>
...
async function handleLoginSubmit(e){
  ...
  try{
    ...
    console.error(err);
    errEl.textContent = 'Tidak bisa terhubung ke server. Periksa SUPABASE_URL/SUPABASE_ANON_KEY di config.js & koneksi internet.';
    errEl.classList.remove('hidden');
    if(btn){ btn.disabled = false; btn.style.opacity = '1'; }
    return;
  }
...
async function initPublicPage(){
  ...
  if(!ok){
    document.getElementById('publicScreen').innerHTML =
      '<div class="p-10 text-center text-sm text-slate-500">Gagal memuat data dari server. Periksa SUPABASE_URL/SUPABASE_ANON_KEY di config.js, lalu muat ulang halaman.</div>';
    return;
  }
