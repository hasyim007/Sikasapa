...
async function getGuruData(db, guruId) {
  ...
  return {
    ...
    db: {
      ...
      ekstraAbsensi: ekstraAbsensi.map((r) => ({ id: r.id, nama: r.nama, keterangan: r.keterangan, warna: r.warna })),
      siswa: siswa.map((r) => ({
...
async function saveAll(env, token, data) {
  ...
  for (const r of (data.ekstraAbsensi || [])) {
    batch1.push(db.prepare(`INSERT INTO ekstra_absensi(id,nama,keterangan,warna) VALUES (?,?,?,?)
      ON CONFLICT(id) DO UPDATE SET nama=excluded.nama, keterangan=excluded.keterangan, warna=excluded.warna`)
      .bind(r.id, r.nama, r.keterangan || '', r.warna || '#1769D1'));
  }
