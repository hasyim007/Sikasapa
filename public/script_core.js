...
function openPemasukanForm(prefill, editId){
  ...
  if(editing){
    ...
    onPmEkskulChange(editing.siswaId);
    ...
    onPmEkskulChange(prefill.siswaId);
...
function onPmEkskulChange(keepSiswaId){
...
function cetakPresensiAbsensiJalankan(){
  ...
  const tanggalList = tanggalPertemuanBulan(ek.hariJadwal, bulan);
  ...
  const baris = peserta.map((s,i)=>`
    <tr>
      ...
      ${tanggalList.map(d=>{
        const a = absensiMap[isoDari(d)+'|'+s.id];
        if(!a) return '<td>&nbsp;</td>';
  ...
  w.document.write(`
    ...
    <body>
      ...
      <div class="legenda">
        <span><strong>H</strong> = Hadir</span><span><strong>I</strong> = Izin</span><span><strong>S</strong> = Sakit</span><span><strong>A</strong> = Alpa</span><span>(kosong = belum diisi guru)</span>
      </div>
      <table>
        <thead>
      ...
      <div>${fieldLabel('Jadwal Hari Latihan')}
        <div class="flex flex-wrap gap-2">
          ${hariAll.map(h=>`<label class="flex items-center gap-1.5 glass px-3 py-1.5 rounded-lg text-xs cursor-pointer">
            <input type="checkbox" value="${h}" ${(e?.hariJadwal||[]).includes(h)?'checked':''} class="ea-hari accent-blue-600"> ${h}
          </label>`).join('')}
        </div>
...
function submitEkstraAbsensi(id){
  ...
  const hariJadwal = Array.from(document.querySelectorAll('.ea-hari:checked')).map(c=>c.value);
...
function deleteEkstraAbsensi(id){
  ...
  showConfirm({
    ...
    message: `"${escapeHtml(nama)}" akan dihapus beserta ${jumlahPeserta} keanggotaan siswa dan ${jumlahAbsensi} catatan absensi terkait. Guru yang ditugaskan ke ekstra ini juga akan kehilangan penugasannya. Tindakan ini tidak bisa dibatalkan.`,
...
function openPesertaAbsensiManage(ekstraId){
  ...
  openModal(`Peserta — ${escapeHtml(ek.nama)}`, `
    <div class="max-h-96 overflow-y-auto space-y-1.5">
      ${siswaAktif.map(s=>`
        <label class="flex items-center gap-2.5 px-2 py-2 rounded-lg hover:bg-slate-50 cursor-pointer">
          <input type="checkbox" class="peserta-absensi-chk accent-blue-600" value="${s.id}" ${(s.ekstraAbsensiIds||[]).includes(ekstraId)?'checked':''}>
  ...
  `, `
    <button onclick="closeModal()" class="btn-secondary px-4 py-2.5 rounded-xl text-sm">Batal</button>
    <button onclick="submitPesertaAbsensi('${ekstraId}')" class="btn-primary px-4 py-2.5 rounded-xl text-sm flex items-center gap-2"><i data-lucide="check" class="w-4 h-4"></i>Simpan</button>
  `);
}
    ...
    <form id="formGuru" class="space-y-4">
      ...
      <div>${fieldLabel('Ditugaskan ke Ekstrakurikuler')}
        <div class="flex flex-wrap gap-2">
          ...
          </label>`).join('') || '<p class="text-xs text-slate-400">Belum ada jenis ekstrakurikuler absensi — tambahkan dulu di menu Kelola Absensi.</p>'}
        </div>
      </div>
