...
function seedDB(){
  return {
    ...
    hapus: { ekskul:[], siswa:[], pemasukan:[], pengeluaran:[], kategoriPengeluaran:[], ekstraAbsensi:[], guru:[], absensi:[] },
    pengaturan: {
      kopLines:[
...
function normalizeDB(db){
  ...
  ['ekskul','siswa','pemasukan','pengeluaran','kategoriPengeluaran','ekstraAbsensi','guru','absensi'].forEach(k=>{
...
/* Tag rentang tanggal yang aman dipakai di nama file, mengikuti filter
   laporanTglAwal/laporanTglAkhir yang sebenarnya dipilih di halaman Laporan
   — dulu nama file PDF selalu memakai currentPeriodeBulan() (bulan kalender
   saat ini), jadi menyesatkan kalau yang diunduh laporan periode lampau. */
function periodeLaporanFileTag(){
  if(!laporanTglAwal && !laporanTglAkhir) return 'semua-periode';
  const awal = laporanTglAwal || 'awal';
  const akhir = laporanTglAkhir || currentPeriodeBulan();
...
function onPmEkskulChange(keepSiswaId, keepPeriode){
  ...
  if(ek.jenisPembayaran === 'bulanan'){
    ...
    let tanggalValid = daftarBulanPm
      .flatMap(ym => tanggalPertemuanBulan(ek.hariJadwal, ym))
      .map(isoDate)
...
function tanggalMulaiTerdeteksi(ek, siswaId){
  ...
  return semua[0] || null; // null = belum ada jejak sama sekali di ekskul ini
}
function hitungEstimasiTunggakanPertemuan(){
  ...
  return ekPertemuan.map(ek=>{
    ...
    const rincian = anggota.map(s=>{
      ...
      let wajibBayar, belumDiisiCount = 0;
      if(ek.ekstraAbsensiId){
        ...
        tanggalRelevan.forEach(tgl=>{
          ...
          if(!a){ wajibBayar++; belumDiisiCount++; } // belum diisi guru -> fallback, asumsi terjadi
          else if(a.status==='hadir') wajibBayar++;
          // izin / sakit / alpa -> tidak dihitung, tidak ditagih
        });
      } else {
        wajibBayar = tanggalRelevan.length;
      }
      const sudahBayar = DB.pemasukan.filter(p=>
        p.ekskulId===ek.id && p.siswaId===s.id && p.jenis==='pertemuan' &&
        daftarBulan.some(ym=>(p.periode||'').startsWith(ym)) && p.periode <= hariIniOnly
      ).length;
      const kurang = Math.max(0, wajibBayar - sudahBayar);
      return { siswa:s, sudahBayar, kurang, wajibBayar, belumDiisiCount };
    ...
    <div class="mt-8">
      <div class="glass-strong rounded-3xl p-5 mb-5 flex items-start gap-4">
        ...
        <div>
          ...
          <p class="text-xs text-slate-500 mt-1">Untuk ekstrakurikuler yang <strong>tertaut ke Kelola Absensi</strong> (lihat label biru), angka di bawah memakai data kehadiran asli — izin/sakit/alpa tidak ikut ditagih. Untuk yang <strong>belum</strong> ditautkan, angka masih murni estimasi dari jadwal hari latihan (dianggap semua pertemuan terjadi) dan bisa meleset kalau siswa izin/sakit/alpa — mohon cek manual sebelum menagih, atau tautkan ekstrakurikulernya ke Kelola Absensi lewat menu edit.</p>
        </div>
      </div>

      ${rekapPertemuan.length===0 ? `
        <p class="text-sm text-slate-500 text-center py-6">Belum ada estimasi tunggakan — semua siswa di ekskul per pertemuan sudah bayar sesuai jumlah pertemuan yang wajib dibayar, atau belum ada pertemuan yang lewat bulan ini.</p>
      ` : `
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 stagger">
          ${rekapPertemuan.map(r=>`
            <div class="glass-strong rounded-3xl p-5 border-t-4" style="border-top-color:${r.ek.warna}">
              <div class="flex items-center justify-between mb-3">
                <div>
                  <h3 class="font-bold text-base flex items-center gap-1.5">${escapeHtml(r.ek.nama)} ${r.ek.ekstraAbsensiId ? `<span class="badge px-2 py-0.5 rounded-full text-[10px] flex items-center gap-1" style="background:rgba(23,105,209,.12); color:var(--blue-600,#1769D1)" title="Memakai data kehadiran asli"><i data-lucide="link-2" class="w-2.5 h-2.5"></i>Tertaut Absensi</span>` : ''}</h3>
                  <p class="text-xs text-slate-500">Tarif: ${rupiah(r.ek.tarif)}/pertemuan · ${r.jumlahPertemuanLewat}x jadwal sudah lewat bulan ini</p>
                </div>
...
/* =========================================================
   ...
   ========================================================= */
let pengingatBulan = null; // diisi ke bulan berjalan saat renderPengingat() pertama kali
...
function kirimPengingatWa(nomor, pesan, namaSiswa){
...
function renderPengingat(){
  const main = document.getElementById('mainContent');
  if(!pengingatBulan) pengingatBulan = currentPeriodeBulan();
  const pg = DB.pengaturan;

  const rekapBulanan = hitungTunggakanBulan(pengingatBulan);
  const totalBulanan = rekapBulanan.reduce((s,r)=>s+r.belum.length,0);

  // Estimasi per pertemuan hanya tersedia untuk bulan BERJALAN (sama
  // seperti menu Tunggakan — perhitungannya berdasar tanggal hari ini).
  const rekapPertemuan = pengingatBulan===currentPeriodeBulan() ? hitungEstimasiTunggakanPertemuan() : [];
  const totalPertemuan = rekapPertemuan.reduce((s,r)=>s+r.rincian.length,0);

  const opsiBulan = [];
  const dNow = hariIniDate();
  for(let i=0;i<6;i++){
    const d = new Date(dNow.getFullYear(), dNow.getMonth()-i, 1);
    opsiBulan.push(d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0'));
  }
  ...
  main.innerHTML = `
    <div class="glass-strong rounded-3xl p-5 mb-5 flex items-start gap-4">
      ...
      <div class="flex-1 min-w-0">
        ...
        <p class="text-xs text-slate-500 mt-1">Pesan disusun otomatis dari template di bawah, tapi tetap perlu Anda tekan tombol Kirim di WhatsApp untuk tiap wali murid (aplikasi tidak terhubung ke WhatsApp Business API). Nomor WA diisi/dikoreksi manual di sini.</p>
      </div>
      <select onchange="pengingatBulan=this.value; renderView('pengingat')" title="Pilih bulan yang dicek" class="glass rounded-xl px-3 py-2 text-sm font-medium shrink-0">
        ${opsiBulan.map(ym=>`<option value="${ym}" ${ym===pengingatBulan?'selected':''}>${bulanNama(ym)}</option>`).join('')}
      </select>
    </div>
    ...
    <details class="glass-strong rounded-3xl p-5 mb-5">
      ...
      <p class="text-xs text-slate-400 mt-3">Placeholder yang bisa dipakai: <code>{namaSiswa}</code> <code>{waliNama}</code> <code>{kelas}</code> <code>{ekskul}</code> <code>{daftarBulan}</code> <code>{totalTunggakan}</code> <code>{jumlahPertemuanKurang}</code> <code>{namaSekolah}</code></p>
    </details>

    <h3 class="text-sm font-bold mb-3 flex items-center gap-2"><i data-lucide="calendar" class="w-4 h-4"></i>Bulanan — ${bulanNama(pengingatBulan)} (${totalBulanan} siswa)</h3>
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 stagger mb-8">
      ${rekapBulanan.length===0 ? `<p class="text-sm text-slate-500 col-span-full text-center py-6">Tidak ada tunggakan bulanan untuk periode ini.</p>` : rekapBulanan.map(r=>`
        <div class="glass-strong rounded-3xl p-5 border-t-4" style="border-top-color:${r.ek.warna}">
          <div class="flex items-center justify-between mb-3">
            <h4 class="font-bold text-base">${escapeHtml(r.ek.nama)}</h4>
            <span class="badge px-2.5 py-1 rounded-full" style="background:rgba(225,29,72,.12); color:var(--rose-500)">${r.belum.length} belum bayar</span>
          </div>
          <div class="divide-y divide-slate-100">
            ${r.belum.map(s=>{
              const pesan = susunPesanPengingat(pg.templateWaBulanan, {
                namaSiswa:s.nama, waliNama:s.waliNama||'Bapak/Ibu', kelas:s.kelas,
                ekskul:r.ek.nama, daftarBulan:bulanNama(r.periode), totalTunggakan:rupiah(r.ek.tarif),
              ...
              <div class="py-2.5 flex items-center gap-2.5 flex-wrap sm:flex-nowrap">
                ...
                <input type="tel" placeholder="08xxxxxxxxxx" value="${escapeHtml(s.waliHp||'')}" ${canEdit()?'':'disabled'}
                  onblur="updateWaliHpDariPengingat('${s.id}', this.value)"
                  class="glass rounded-lg px-2.5 py-1.5 text-xs w-32 shrink-0" title="Nomor WA wali murid ${escapeHtml(s.nama)}">
                ${canEdit() ? `<button onclick='kirimPengingatWa(${JSON.stringify(s.waliHp||"")}, ${JSON.stringify(pesan)}, ${JSON.stringify(s.nama)})' class="btn-primary px-3 py-1.5 rounded-lg text-xs flex items-center gap-1.5 shrink-0"><i data-lucide="send" class="w-3.5 h-3.5"></i>Kirim</button>` : ''}
              </div>`;
            }).join('')}
          </div>
        </div>
    ...
    ${DB.ekskul.some(e=>e.jenisPembayaran==='pertemuan') ? `
    <h3 class="text-sm font-bold mb-3 flex items-center gap-2"><i data-lucide="calendar-clock" class="w-4 h-4"></i>Per Pertemuan — estimasi bulan berjalan (${totalPertemuan} siswa)</h3>
    ${pengingatBulan!==currentPeriodeBulan() ? `<p class="text-xs text-slate-500 mb-4">Estimasi per pertemuan hanya dihitung untuk bulan berjalan. Pilih "${bulanNama(currentPeriodeBulan())}" di atas untuk melihatnya.</p>` : `
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 stagger">
      ${rekapPertemuan.length===0 ? `<p class="text-sm text-slate-500 col-span-full text-center py-6">Tidak ada estimasi kekurangan bayar per pertemuan bulan ini.</p>` : rekapPertemuan.map(r=>`
        <div class="glass-strong rounded-3xl p-5 border-t-4" style="border-top-color:${r.ek.warna}">
          <div class="flex items-center justify-between mb-3">
            <h4 class="font-bold text-base">${escapeHtml(r.ek.nama)}</h4>
          ...
          <div class="divide-y divide-slate-100">
            ${r.rincian.map(item=>{
              const pesan = susunPesanPengingat(pg.templateWaPertemuan, {
                ...
                ekskul:r.ek.nama, daftarBulan:bulanNama(currentPeriodeBulan()),
              ...
              <div class="py-2.5 flex items-center gap-2.5 flex-wrap sm:flex-nowrap">
                ...
                <input type="tel" placeholder="08xxxxxxxxxx" value="${escapeHtml(item.siswa.waliHp||'')}" ${canEdit()?'':'disabled'}
                ...
                ${canEdit() ? `<button onclick='kirimPengingatWa(${JSON.stringify(item.siswa.waliHp||"")}, ${JSON.stringify(pesan)}, ${JSON.stringify(item.siswa.nama)})' class="btn-primary px-3 py-1.5 rounded-lg text-xs flex items-center gap-1.5 shrink-0"><i data-lucide="send" class="w-3.5 h-3.5"></i>Kirim</button>` : ''}
              </div>`;
            }).join('')}
          </div>
        </div>
...
function cetakPresensiAbsensiJalankan(){
  ...
  const ek = ekstraAbsensiById(ekstraId);
  const pg = DB.pengaturan;
  if(!ek){ showToast('Ekstrakurikuler tidak ditemukan.', 'error'); return; }
  if(!Array.isArray(ek.hariJadwal) || ek.hariJadwal.length===0){
    showToast('Jadwal hari latihan belum diatur untuk ekstrakurikuler ini.', 'error'); return;
  }
  const peserta = pesertaAbsensi(ekstraId).slice().sort((a,b)=>a.nama.localeCompare(b.nama,'id'));

  // Bangun string tanggal ISO manual dari komponen Date (BUKAN
  // toISOString()) — sama alasannya seperti hariIniStr(): Date di sini
  // sudah berupa tanggal lokal apa adanya, toISOString() akan salah
  // konversi kalau dipaksa lewat UTC.
  const isoDari = d => d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0');

  // Kolom tanggal = jadwal hari latihan (seperti biasa) DIGABUNG tanggal
  // mana pun yang BENAR-BENAR punya catatan absensi di bulan ini walau
  // jatuh di luar hari jadwal (guru salah pilih tanggal, atau ini memang
  // pertemuan pengganti/tambahan — lihat peringatan di form isi absensi).
  // Tanpa penggabungan ini, tanggal di luar jadwal tersimpan di database
  // tapi TIDAK PERNAH tercetak di lembar manapun — data seolah "hilang".
  const tanggalJadwal = tanggalPertemuanBulan(ek.hariJadwal, bulan);
  ...
  if(tanggalList.length===0){ showToast('Tidak ada tanggal pertemuan pada bulan yang dipilih.', 'error'); return; }
  if(peserta.length===0){ showToast('Belum ada siswa terdaftar di ekstrakurikuler ini.', 'error'); return; }

  const statusSingkat = { hadir:'H', izin:'I', sakit:'S', alpa:'A' };
  const absensiMap = {}; // key: tanggal|siswaId -> record
  ...
  const kolomTanggal = tanggalList.map(t=>`<th${t.diLuarJadwal?' style="background:#FEF3C7"':''} title="${t.diLuarJadwal?'Di luar hari jadwal latihan':''}">${HARI_PENDEK[t.dow]}<br>${String(t.tgl).padStart(2,'0')}/${String(t.bln).padStart(2,'0')}${t.diLuarJadwal?' *':''}</th>`).join('');
  ...
  w.document.write(`
    ...
    <style>
      ...
      .legenda{font-size:10.5px; color:#555; margin-bottom:10px;}
    ...
    <body>
      ...
      <div class="legenda">
        <span><strong>H</strong> = Hadir</span><span><strong>I</strong> = Izin</span><span><strong>S</strong> = Sakit</span><span><strong>A</strong> = Alpa</span><span>(kosong = belum diisi guru)</span>${jumlahDiLuarJadwal>0 ? `<span><strong>*</strong> = tanggal di luar hari jadwal latihan (tetap dicetak supaya tidak ada data hilang)</span>` : ''}
      </div>
      <table>
        <thead>
...
function submitEkskul(id){
  ...
  if(id){
    ...
    DB.ekskul.push({ id:uid('ek'), nama, pembina, jenisPembayaran, tarif, warna, hariJadwal, ekstraAbsensiId });
...
function submitSiswa(id){
  ...
  if(id){
    ...
    siswaTersimpan = { id:uid('sw'), nama, kelas, waliNama, waliHp, ekskulIds, aktif, ekstraAbsensiIds:[] };
...
async function cetakLaporan(ekskulId){
  ...
  const anggota = DB.siswa.filter(s=>s.ekskulIds.includes(ekskulId) && s.aktif!==false);
...
/* Menghasilkan array objek Date untuk setiap tanggal di bulan `ym` (format
   ...
   (DB.pengaturan.hariLibur, mis. libur nasional/cuti bersama) — dipakai
   bersama oleh Estimasi Tunggakan Per Pertemuan & Cetak Presensi supaya
   keduanya konsisten: hari libur tidak dihitung sebagai pertemuan yang
   "seharusnya ada", jadi tidak ikut menagih & tidak muncul di lembar presensi. */
function tanggalPertemuanBulan(hariJadwal, ym){
  ...
  const libur = new Set((DB.pengaturan && Array.isArray(DB.pengaturan.hariLibur)) ? DB.pengaturan.hariLibur : []);
...
function renderPengaturan(){
  ...
  main.innerHTML = `
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
      ...
      <div class="glass-strong rounded-3xl p-6 lg:col-span-2">
        <h3 class="font-bold text-sm mb-1.5 flex items-center gap-2"><i data-lucide="calendar-off" class="w-4 h-4" style="color:var(--amber-400)"></i>Hari Libur Ekstra</h3>
        <p class="text-xs text-slate-500 mb-4">Tanggal yang ditandai di sini <b>tidak ikut dihitung</b> sebagai pertemuan pada Estimasi Tunggakan Per Pertemuan &amp; tidak muncul sebagai kolom di Cetak Presensi — untuk libur nasional, cuti bersama, atau libur sekolah lain di luar akhir pekan. Perubahan di sini tersimpan langsung, tidak perlu klik "Simpan Semua Pengaturan".</p>
        <div class="flex flex-col sm:flex-row gap-2 mb-4">
          <input id="pgHariLiburBaru" type="date" class="input-glass rounded-xl px-3.5 py-2.5 text-sm">
          <button onclick="tambahHariLibur()" class="btn-primary px-4 py-2.5 rounded-xl text-sm shrink-0">Tandai Libur</button>
        </div>
        <div id="hariLiburList" class="flex flex-wrap gap-2">
          ${pg.hariLibur.length ? pg.hariLibur.map(t=>`
            <span class="badge px-2.5 py-1.5 rounded-full glass flex items-center gap-1.5">${escapeHtml(tanggalIndo(t))}
              <button onclick='hapusHariLibur(${JSON.stringify(t)})' title="Hapus tanggal libur" aria-label="Hapus tanggal libur ${escapeHtml(tanggalIndo(t))}" class="text-slate-500 hover:text-rose-500"><i data-lucide="x" class="w-3 h-3"></i></button>
            </span>
          `).join('') : '<p class="text-xs text-slate-400">Belum ada hari libur ekstra yang ditandai.</p>'}
        </div>
      </div>
  ...
  reader.onload = ()=>{
    ...
    showConfirm({
      ...
      onConfirm: async ()=>{
        ...
        const berhasil = await restoreDB(DB);
        if(berhasil){
...
/* =========================================================
   HARI LIBUR EKSTRA — dikecualikan dari perhitungan pertemuan
   (Estimasi Tunggakan Per Pertemuan & Cetak Presensi), lihat
   tanggalPertemuanBulan(). Tersimpan langsung per perubahan,
   sama seperti Kategori Pengeluaran — bukan bagian dari form
   "Simpan Semua Pengaturan" di bawahnya.
   ========================================================= */
function tambahHariLibur(){
  if(!requireEdit()) return;
  const input = document.getElementById('pgHariLiburBaru');
  const val = input.value;
  if(!val) return;
  const pg = DB.pengaturan;
  if(!Array.isArray(pg.hariLibur)) pg.hariLibur = [];
  if(pg.hariLibur.includes(val)){ showToast('Tanggal itu sudah ditandai sebagai hari libur.', 'error'); return; }
  pg.hariLibur.push(val);
  pg.hariLibur.sort();
  catatAktivitas('Tambah Hari Libur', tanggalIndo(val));
  saveDB(DB);
  renderView('pengaturan');
}
function hapusHariLibur(tanggal){
  if(!requireEdit()) return;
  const pg = DB.pengaturan;
  pg.hariLibur = (pg.hariLibur||[]).filter(t=>t!==tanggal);
  catatAktivitas('Hapus Hari Libur', tanggalIndo(tanggal));
  saveDB(DB);
  renderView('pengaturan');
}
