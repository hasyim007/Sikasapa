...
const MENUS = [
  ...
  { id:'absensi', label:'Kelola Absensi', icon:'clipboard-list', subtitle:'Absensi ekstrakurikuler oleh guru ekstra', roles:['bendahara','kepsek','guru'] },
...
function onAbsensiStatusChange(sel){
  const st = STATUS_ABSENSI.find(s=>s.v===sel.value);
  sel.style.background = st ? st.color : '';
  sel.style.color = '#fff';
}
    ...
    <div class="rounded-2xl overflow-hidden" style="border:1px solid rgba(148,163,184,.18)">
      <div class="divide-y divide-slate-100">
        ${peserta.map(s=>{
          ...
          return `
          <div class="flex items-center gap-3 px-4 py-3 flex-wrap" data-absensi-row="${s.id}">
            <div class="flex-1 min-w-[140px]">
              <p class="text-sm font-medium text-slate-800">${escapeHtml(s.nama)}</p>
              <p class="text-xs text-slate-400">${escapeHtml(s.kelas)}</p>
            </div>
            <div class="flex items-center gap-1 shrink-0">
              <select data-status="${s.id}" ${bolehIsi?'':'disabled'} onchange="onAbsensiStatusChange(this)"
                style="background:${warnaStatusAbsensi(cur.status)};color:#fff" class="rounded-lg px-2.5 py-1.5 text-[11px] font-semibold border-0 outline-none cursor-pointer transition">
                ${STATUS_ABSENSI.map(st=>`<option value="${st.v}" ${cur.status===st.v?'selected':''}>${st.label}</option>`).join('')}
              </select>
            </div>
...
async function simpanAbsensi(ekstraId, tanggal){
  ...
  const catatan = peserta.map(s=>{
    const sel = document.querySelector(`select[data-status="${s.id}"]`);
    const catatanEl = document.querySelector(`input[data-catatan="${s.id}"]`);
    return { siswaId:s.id, status: sel ? sel.value : 'hadir', catatan: catatanEl ? catatanEl.value.trim() : '' };
  });
...
function submitEkstraAbsensi(id){
  ...
  if(id){
    Object.assign(ekstraAbsensiById(id), { nama, keterangan, warna });
    catatAktivitas('Ubah Jenis Ekstrakurikuler Absensi', nama);
  } else {
    const baru = { id:uid('ea'), nama, keterangan, warna };
