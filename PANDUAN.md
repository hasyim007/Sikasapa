...
1. Bendahara menambahkan **jenis ekstrakurikuler absensi** (mis. Pramuka,
   Rebana) di menu Kelola Absensi, lalu mendaftarkan siswa pesertanya
   lewat tombol "Kelola Peserta".
...
4. Guru memilih tanggal, menandai status tiap siswa (Hadir/Izin/Sakit/
   Alpa), lalu Simpan. Mengisi ulang tanggal yang sama akan menimpa data
   sebelumnya (bukan dobel).
...
```
Untuk instalasi baru dari nol, `schema.sql` sudah termasuk semua tabel
di atas — migrasi ini tidak perlu dijalankan.
