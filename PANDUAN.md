...
## Fitur: Pengingat Pembayaran (WhatsApp)
...
- Template pesan bisa diubah bebas lewat panel "Atur Template Pesan" —
  ...
  `{totalTunggakan}`, dst yang otomatis diganti data sebenarnya.
...
4. Guru mengikuti alur 2 langkah di layar: (1) pilih ekstrakurikuler &
   tanggal, (2) tandai status tiap siswa lewat tombol Hadir/Izin/Sakit/
   ...
   tanggal yang sama akan menimpa data sebelumnya (bukan dobel).
5. **Cetak Presensi** (tombol di atas daftar siswa) mencetak lembar
   ...
   latihan, dan tiap sel SUDAH terisi H/I/S/A langsung dari absensi yang
   diisi guru di langkah 4 — bukan lembar kosong yang harus ditulis
   ulang manual. Tersedia untuk Bendahara, Kepala Sekolah, maupun Guru
   Ekstra sendiri.
...
Cetak Presensi di atas. Untuk instalasi baru dari nol, `schema.sql`
sudah termasuk semua tabel & kolom di atas — kedua migrasi ini tidak
perlu dijalankan.
